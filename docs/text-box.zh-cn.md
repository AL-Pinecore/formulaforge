...
- `app/editor/TextController.ts` — text 原子读取、删除/输入重建、命中与字体样式
- `app/components/EquationWorkspace.vue` — 键盘事件编排、历史发布与可视 overlay
...
MathLive 把每个文本字符序列化成独立的 `\text{<char>}` 原子，单原子文本命令会被折叠、`\text{}` 包装丢失。所以 `TextController` 拦截文本上下文里的可打印字符和删除键，自己重建整组：

- `textGroupFromAtom` 按「模式 + 样式」的 run key 收集连续文本原子，拼出内容和 `\text{...}` 组范围；
- `handleTextInput` / `handleTextDeletion` 取 `group.content`、插入/删除字符、`mf.insert(\<command>{<content>})` 整组替换，再用 `placeCaretInTextGroup` 把光标放回原字符后；返回 `changed` / `handled` / `continue` 让工作区决定是否发布历史。
...
- 清空整个输入框不会重置 MathLive 的模型 `mode`（`math`/`text`/`latex`），光标曾在文本框内时清空会残留 `text` 模式，导致下一次输入被包进 `\text{}`。`MathLiveAdapter.ensureMathMode` 在内容为空时把它重置回 `math`（只重置 `text`，不碰进行中的反斜杠命令 `latex` 模式）。
